    ...
    <div className="page-enter">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-8 w-full pb-24 md:pb-32">
          <div className="max-w-3xl">
            ...
            <div className="flex items-center gap-5 mt-8 mb-4">
              <div className="w-14 h-14 bg-white/10 text-white rounded-2xl flex items-center justify-center border border-white/10"><Icon size={28} /></div>
      ...
      <section className="py-12 sm:py-16 md:py-20 lg:py-28 px-4 sm:px-6 md:px-8 bg-[#FDFBF7] relative">
        <div className="max-w-4xl sm:max-w-5xl lg:max-w-6xl mx-auto relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {data.benefits.map((b: string, i: number) => (
                <AnimateIn key={i} delay={0.1 + i * 0.08}>
                  <div className="flex items-start gap-4 bg-white p-6 rounded-2xl border border-[var(--border)] shadow-sm hover:border-[var(--brand)] hover:shadow-md transition-all duration-300">
          ...
          {/* Process Steps - Solid numbered cards */}
          {steps.length > 0 && (
            <div className="mb-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {steps.map((step: string, i: number) => (
                  <AnimateIn key={i} delay={0.1 + i * 0.08}>
                    <div className="bg-white p-5 rounded-2xl border border-[var(--border)] shadow-sm text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
                      <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 text-sm font-bold text-white bg-[var(--brand)]">
                        {i + 1}
                      </div>
          ...
          {/* CTA - Solid clean corporate style */}
          <div className="relative bg-[var(--dark)] p-6 sm:p-8 md:p-10 lg:p-14 rounded-3xl text-center overflow-hidden border border-white/5">
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap items-center justify-center gap-4">
                <Link href={`/${lang}/contact`} className="bg-white text-[#1A252F] px-8 py-4 rounded-xl font-bold hover:bg-[var(--brand)] hover:text-white transition-all">{lang === 'id' ? 'Hubungi Kami' : 'Contact Us'}</Link>
                <Link href={`/${lang}/services`} className="text-white/80 hover:text-white px-6 py-4 rounded-xl font-semibold border border-white/20 hover:border-white/40 transition-all flex items-center gap-2"><ChevronLeftIcon size={16} /> {lang === 'id' ? 'Semua Layanan' : 'All Services'}</Link>
              </div>
            </div>
          </div>
        </div>
